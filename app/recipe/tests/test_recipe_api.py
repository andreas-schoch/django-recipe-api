...
class PrivateIngredientsApiTests(TestCase):
    ...
    def test_filter_recipes_by_tags(self):
        ...
        recipe1 = sample_recipe(user=self.user, title='bla1')
        recipe2 = sample_recipe(user=self.user, title='bla2')
        tag1 = sample_tag(user=self.user, name='tag1')
        tag2 = sample_tag(user=self.user, name='tag2')
        recipe1.tags.add(tag1)
        recipe2.tags.add(tag2)
        recipe3 = sample_recipe(user=self.user, title='bla3')

        # Adding a payload to a GET request should return first two recipes in this case
...
class RecipeImageUploadTests(TestCase):
    ...
    def test_upload_image_bad_request(self):
        ...
        url = image_upload_url(self.recipe.id)
        res = self.client.post(url, {'image': 'is no image'}, format='multipart')
